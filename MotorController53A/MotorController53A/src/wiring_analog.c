...
/*void analogWrite(uint8_t pin, int val)
{
  ...
  // Avoid digital write and make do with less precise PWM based 0 and 255.
  if (val <= 0)
  {
  ...
  //else
  {
    ...
    {
      // Just consider it an error to analogWrite a non pwm pin.
      // We want to trim down on the size here... (save 10 bytes)
      //if (val < 128)
      //{
      //  digitalWrite(pin, LOW);
      //}
